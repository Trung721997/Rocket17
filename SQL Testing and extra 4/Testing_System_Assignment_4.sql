...
SELECT		D.DepartmentName, A.Email, A.Username
...
HAVING		count(EQ.QuestionID) IN (SELECT 		max(A)
...
SELECT		CQ.CategoryID, CQ.CategoryName, count(Q.CategoryID) AS SoLuong
...
SELECT		Q.QuestionID, Q.Content, count(Q.QuestionID) AS SoLuong
FROM		Question AS Q
JOIN		ExamQuestion AS EQ
ON			Q.QuestionID = EQ.QuestionID
GROUP BY	EQ.QuestionID
...
SELECT		 Q.Content, count(AR.QuestionID) AS SoLuong
FROM		Question AS Q
JOIN		Answer AS AR
ON			Q.QuestionID = AR.QuestionID
GROUP BY	AR.QuestionID
HAVING		count(AR.QuestionID) IN (SELECT		max(A)
 ...
 HAVING		count(A.PositionID) IN (SELECT	min(X)
